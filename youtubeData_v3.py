...
def write_to_excel_content(content_list):
    if os.path.exists('./contentData.xlsx'):
        wb = openpyxl.load_workbook('contentData.xlsx')
        worksheet = wb.active
        for listx in content_list:
            
            worksheet.append(listx)
            
        wb.save('contentData.xlsx')
        wb.close()
    else:
        wb = openpyxl.Workbook()
        dest_filename = 'contentData.xlsx'
        worksheet = wb.active
...
def write_to_excel(d_list):
    if os.path.exists('./commentData.xlsx'):
        wb = openpyxl.load_workbook('commentData.xlsx')
        worksheet = wb.active
        for listx in d_list:
            
            worksheet.append(listx)
            
        wb.save('commentData.xlsx')
        wb.close()
    else:
        wb = openpyxl.Workbook()
        dest_filename = 'commentData.xlsx'
        worksheet = wb.active
...
# =============================================================================
#     workbook = xlsxwriter.Workbook('version2data.xlsx') 
#   
# 
#     worksheet = workbook.add_worksheet() 
#   
# 
#     worksheet.write('A1', 'vid') 
#     worksheet.write('B1', 'vdate')
#     worksheet.write('C1', 'comment_id') 
#     worksheet.write('D1', 'comment') 
#     worksheet.write('E1', 'channel_id') 
#     worksheet.write('F1', 'like_count') 
#     worksheet.write('G1', 'published_date') 
#     worksheet.write('H1', 'updated_date') 
#    # worksheet.write('H1', 'channel_id')
#     count = 1
#     for listx in d_list:
#         
#         for j in range(0,len(listx)):
#             
#             worksheet.write(count,j, listx[j]) 
#         count = count + 1
...
def convert_audio_to_text(filepath, chunksize=60000):
    ...
    print(f"{len(chunks)} chunks of ovindu {chunksize/1000}s each")
    ...
    try:
        ...
        for index,chunk in enumerate(chunks):
            #TODO io.BytesIO()
            chunk.export('E:/spam_classifier_sub_module/alfa.wav', format='wav')
            with sr.AudioFile('E:/spam_classifier_sub_module/alfa.wav') as source:
